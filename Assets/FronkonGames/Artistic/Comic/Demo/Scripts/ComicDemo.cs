...
public class ComicDemo : MonoBehaviour
{
  ...
  private void Awake()
  {
    ...
    volume = volumeProfile != null && volumeProfile.TryGet(out ComicVolume vol) ? vol : null;
    ...
    GUILayout.BeginHorizontal();
    {
      GUILayout.BeginVertical("box", GUILayout.Width(450.0f), GUILayout.Height(Screen.height));
      {
        ...
        if (volume != null)
        {
          ...
          volume.scale.value = SliderField("Scale", volume.scale.value, 0.01f, 1.0f);
          volume.colorBlend.value = EnumField("Color blend", volume.colorBlend.value);

          GUILayout.Space(space);

          volume.edge.value = SliderField("Edge", volume.edge.value, 0.0f, 10.0f);
          volume.edgeColor.value = ColorField("Edge color", volume.edgeColor.value, false);
          volume.edgeColorBlend.value = EnumField("Edge blend", volume.edgeColorBlend.value);

          GUILayout.Space(space);

          GUILayout.Label("CMYK pattern", styleLabel);
          volume.cmykPattern.value = Vector4Field(volume.cmykPattern.value, 0.0f, 90.0f);
        }
  ...
  private Vector4 Vector4Field(Vector4 value, float min, float max)
  {
    value.x = SliderField("   Cyan", value.x, min, max);
    value.y = SliderField("   Magenta", value.y, min, max);
    value.z = SliderField("   Yellow", value.z, min, max);
    value.w = SliderField("   Black", value.w, min, max);
