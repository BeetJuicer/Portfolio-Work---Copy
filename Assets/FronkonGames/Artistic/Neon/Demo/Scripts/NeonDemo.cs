...
public class NeonDemo : MonoBehaviour
{
  ...
  private void Awake()
  {
    ...
    volume = volumeProfile != null && volumeProfile.TryGet(out NeonVolume vol) ? vol : null;
    ...
    GUILayout.BeginHorizontal();
    {
      GUILayout.BeginVertical("box", GUILayout.Width(450.0f), GUILayout.Height(Screen.height));
      {
        ...
        if (volume != null)
        {
          ...
          volume.strength.value = SliderField("Strength", volume.strength.value);
          volume.radius.value = SliderField("Radius", volume.radius.value, 1, 20);
          volume.blend.value = EnumField("Blend", volume.blend.value);
          volume.fisheye.value = SliderField("Fisheye", volume.fisheye.value, -1.0f, 1.0f);
          volume.speed.value = SliderField("Speed", volume.speed.value, 0.0f, 5.0f);

          GUILayout.Space(space);

          volume.processDepth.value = ToggleField("Process depth", volume.processDepth.value);
          if (volume.processDepth.value == true)
          {
            volume.depthPower.value = SliderField("   Power", volume.depthPower.value);
            volume.sampleSky.value = ToggleField("   Sample sky", volume.sampleSky.value);
          }
        }
